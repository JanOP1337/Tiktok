...
public:
	...
	byte ShiftRegisterOutputHolder = 0b1100;
...
//JOP
    /// Called to execute a step on a 4 pin motor with shift register. Only called when a new step is
    /// required. 
    virtual void   step4S(long step);

    /// Called to execute half a step on a 4 pin motor with shift register. Only called when a new step is
    /// required. 
    virtual void   step8S(long step);
//JOP
    /// Called to execute a step on a 3 pin motor, such as HDD spindle. Only called when a new step is
    /// required. Subclasses may override to implement new stepping
    /// interfaces. The default sets or clears the outputs of pin1, pin2,
